...
impl MmapMut {
  ...
  #[inline]
  pub fn open_with_options<P: AsRef<std::path::Path>>(path: P, open_opts: OpenOptions) -> Result<Self> {
  ...
  fn open_in(path: &std::path::Path, open_opts: OpenOptions) -> Result<Self> {
    open_opts.open(path).map_err(From::from).and_then(|file| {
      ...
      unsafe { Map::map_mut(&file) }.map(|map| Self { inner: Inner::new(file, len), map }).map_err(Error::Mmap)
    })
  }
  ...
  pub fn freeze(self) -> Result<super::immutable::Mmap> {
    ...
    self.map.make_read_only().map(|map| {
      super::immutable::Mmap {
        inner: self.inner,
        map,
        len,
      }
